...
```
benches/        symlinks to the three working checkouts (edit in place)
...
5. **Sessions started at this root** may read everything and write to
   `artifacts/`, `letters/`, and the bench named by the task — never to a
   bench not named. Sessions started inside a bench obey that bench's
   `AGENTS.md` (including its §0 partnership rules).
