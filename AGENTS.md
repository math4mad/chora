...
> 主人 0925 凌晨定纲：「遵循最开始落下的原则，都是对问题的回答……实验细节就成了支柱而不是大梁」；三梁图+立梁门同日朱批。

**大梁三根**（每根 = 园子对世界的一问，自始只有这些；实验是销钉，负读数也是承重）：

| 梁 | 一问 | 立梁处 | 已焊销钉（含负） |
|---|---|---|---|
| **梁一·基与空间** | 每换一组 (α,β) 就是一个空间；界由人划、理由由证据选 | JacobiGP | exp1–5；H6a/b/c（H6c 负读数=梁在受力，非塌梁） |
| **梁二·中心与区域** | 模型中间层是否就是泛化能力层（概念=带中心的低秩子空间，探针塌缩区域） | MEF | exp10 词表翻转、iso-audit、M案聋盲臂 |
| **梁三·序与生长** | 喂入之序是否自身在参数里留痕（课程非快法，乃长结构之慢法） | corpus/AGE-LADDER + Kairos(onset 归此梁：onset 是先验落定的参数，非第四旋钮) | LADDER-2b 双判据 3/3 PASS；PB16 认祖此梁 |

**柱**（各答一问，本分，不得冒充梁）：Sarcos=快控台面上带结构可见否（兼台架：机器效度数出自它）；
PolyNN=层的形状里住不住 P-structure（exp8 已答）；xdom/PB15=语料制式对照 apparatus；
Theuth/Lyceum=账目可否自验；vault/queue/push=物流，不配问题。
**游柱封账**：G1 垃圾基因两源案及诸生物学类比支线——无梁可挂，就地封账不续浇（册面存留，判据不废，新弹不发）。

**立梁门**（新案预注册必多填一行，写不出者自动降柱，柱不占晨圈点菜名额）：
> 「本问若答『是』，梁几加一销；若答『否』，梁几裂何处。」

此门与既有三件套同源：判据先冻、数字对 sha、不裸引——今加「问题挂梁」为第四件。
