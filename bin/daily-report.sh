...
#   morning : 看板未决 + 提醒未勾 → 逐条挂进「园区·GRAPHIA」(去重), 勾掉即批复
...
have=subprocess.run(["osascript","-e",'tell application "Reminders" to tell list "园区·GRAPHIA" to get name of reminders whose completed is false'],capture_output=True,text=True).stdout
for r in pick:
...
import datetime; print('晨圈' in out and datetime.date.today().isoformat()[5:] in out)" | grep -q True; then
  ...
  /opt/miniconda3/envs/default/bin/python "$ROOT/bin/daily_note.py" "$DIG" "$D" >> $LOG 2>&1 || echo "notes channel failed (不阻断邮件与图档)" >> $LOG
  rm -f $DIG
fi
